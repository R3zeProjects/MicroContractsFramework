...
6. совместно запускает configuration, error, logging, persistence, telemetry,
   resilience и child-process workflow;
...
`FrameworksRoot` должен содержать каталоги шести runtime-репозиториев. Каталог сборки
