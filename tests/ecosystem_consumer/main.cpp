...
static_assert(vosp::protocol::version::api == "0.2.0-beta");
static_assert(vosp::transport::version::api == "0.2.0-beta");
...
class MemoryJournal {
public:
  [[nodiscard]] vosp::persistence::OperationResult
  append(const vosp::persistence::Record &record) {
...
public:
  [[nodiscard]] vosp::persistence::OperationResult
  append(const vosp::persistence::Record &) {
...
public:
  ...
  [[nodiscard]] bool
  export_batch(std::span<const vosp::telemetry::Record> records) override {
...
public:
  explicit ConfigurationObserver(Pipeline &pipeline) noexcept
      : pipeline_{&pipeline} {}
...
[[nodiscard]] bool
validate_process_and_resilience(const std::filesystem::path &executable) {
  ...
  const auto failed_process =
      runner.run({.executable = executable,
                  .arguments = {"--ecosystem-child-fail"},
                  .working_directory = std::nullopt});
  if (!failed_process || failed_process->succeeded() ||
      failed_process->exit_code != 7) {
    return false;
  }
  ...
  const auto retried = retry.execute([&]() -> vosp::error::Result<int> {
    if (++attempts < 3) {
      return std::unexpected{vosp::error::Error{vosp::error::Category::NETWORK,
...
[[nodiscard]] bool validate_data_plane() {
  ...
  vosp::TelemetryPipeline<vosp::telemetry::pipeline_policy::Direct> pipeline{
      exporter};
  ...
  const vosp::error::Error error{vosp::error::Category::DATABASE, 1001,
                                 "configuration changed"};

  if (!configuration.publish_and_notify(layers, observer) ||
      !observer.published() || !logger.error(error) || !changes.add() ||
      !pipeline.flush()) {
    return false;
  }
...
[[nodiscard]] bool validate_failure_paths() {
  ...
  schema.require<std::int64_t>("service.port", [](std::int64_t port) {
  ...
  const auto rejected_configuration =
      configuration.publish(invalid_layers, schema);
...
[[nodiscard]] bool validate_service_control_plane() {
  ...
  return events == std::vector<std::string>{"start:database", "start:api",
                                            "stop:api", "stop:database"};
}
...
[[nodiscard]] bool validate_transport_plane() {
  ...
  return !stream.connected() && !socket.open() && !invalid_connect &&
         !invalid_receive &&
         invalid_connect.error().kind() ==
             vosp::transport::ErrorCode::invalid_argument &&
         invalid_receive.error().kind() ==
             vosp::transport::ErrorCode::not_connected;
}
} // namespace
